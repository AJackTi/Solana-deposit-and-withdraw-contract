...
export type AccountNameAndDiscriminator = {
  name: string;
  discriminator: Buffer;
};
export function createAccountDiscriminators(
  idl: Idl
): Array<AccountNameAndDiscriminator> {
...
export async function getAccountsData({
  ...
  const accountsData = chain(publicKeysAndInfo)
    ...
    .map(([publicKey, accountInfo]): AccountData => {
      ...
      const accountName =
        getAccountName(ACCOUNTS_NAMES_AND_DISCRIMINATORS, data) ?? "";

      const parsedData = decodeAccountDataSafe<unknown>(
        coder,
        accountName,
        data
      );
...
export function parseEnumsInAccount<AccountDataType>(
  rawAccount: any
): AccountDataType {
