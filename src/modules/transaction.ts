...
export function getProgramInstructionsFromParsedTransaction(
  ...
  return (
    chain(transaction.message.instructions)
      ...
      .filter(
        (ixn) =>
          !!(ixn as web3.PartiallyDecodedInstruction)?.data &&
          ixn.programId.equals(programId)
      )
...
export function getInstructionAccountsSafe(
  ...
  try {
    ...
    if (!idlIxn)
      throw new Error("Provided IDL doesn't contain this instruction");
...
export function getProgramAccountsFromParsedTransaction({
  ...
  const programInstructions = getProgramInstructionsFromParsedTransaction(
    programId,
    transaction
  );
...
export function getProgramAccountsFromMessage({
  ...
  //? Get instructions that relate only to provided program
  const programInstructions = getProgramInstructionsFromMessage(
    programId,
    message
  );
  ...
  return chain(programInstructions)
    .map((ixn) => {
      const decodedIxn = createPartiallyDecodedInstruction(
        staticAccountKeys,
        ixn
      );
...
export function getProgramAccountsFromHeliusEnhancedTransaction({
  ...
  const programInstructions =
    getProgramInstructionsFromHeliusEnhancedTransaction(programId, transaction);
...
export async function confirmTransactionByPollingSignatureStatus({
  ...
  commitment,
  refetchInterval,
}: ConfirmTransactionByPollingSignatureStatusParams): Promise<
  string | undefined
> {
  try {
    while (!abortController.signal.aborted) {
      const { value: signatureValue } = await connection.getSignatureStatus(
        signature,
        {
